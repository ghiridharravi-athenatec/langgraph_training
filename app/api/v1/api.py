...
from app.utils.retrieve import compiled_graph, embedding_model
from app.core import config
from app.core.logger import get_logger
from app.core.guardrails import validate_input, evaluate_intent_detection, validate_collection_authorization, validate_quota
...
def _run_with_timeout(fn, *args, stage: str, timeout_seconds: int = config.REQUEST_TIMEOUT_SECONDS, **kwargs):
...
async def ingest(
    ...
    collection_name: str = "warranty",
    current_user: dict = Depends(_require_ragchatbot_access),
    _rate_limit_check: dict = Depends(_ingest_rate_limit),
):
    '''
    Ingest a file (PDF or XLSX) into the specified mongodb collection. The file is saved in the uploads directory and then processed for ingestion.\n
    Collection name must be one of: warranty, user_manual, inspection_report.
    '''
    try:
        if collection_name not in ["warranty", "user_manual", "inspection_report"]:
            logger.error("Invalid collection name provided: '%s'", collection_name)
            raise HTTPException(status_code=400, detail="Invalid collection name. Must be one of: warranty, user_manual, inspection_report.")
        ...
        ingest = ingest_files([str(file_path)], collection_name=collection_name)

        if ingest["passed"]:
            logger.info("Ingestion succeeded for '%s' into collection '%s'", file.filename, collection_name)
        else:
            logger.error("Ingestion failed for '%s' into collection '%s': %s", file.filename, collection_name, ingest["error"])
...
def _build_graph_response(state: QAResponse, compile: dict = None, extra_guardrail_events: list = None) -> dict:
    ...
    compile = compile or {}
    return {
        "collection_name": compile.get("collection_name", state.collection_name),
...
def _persist_turn(
    ...
    future cache candidate (see mongo.list_cache_candidates). Also stamps conversation_id onto the
    response dict in place, so the caller (which may have auto-created this conversation) always
    tells the frontend which conversation the turn landed in.'''
...
async def chat_with_document(
    ...
    try:
        ...
        if not input_check["passed"]:
            ...
            _persist_turn(conversation_id, current_user["_id"], original_question, response, blocked=True)
            return response
        state.question = input_check["sanitized_question"]

        today = date.today().isoformat()
        quota_event = validate_quota(
            get_daily_usage(current_user["_id"], today), current_user.get("role") == ROLE_ADMIN
        )
        if not quota_event["passed"]:
            ...
            response = {
                ...
                "graph_response": _build_graph_response(state, extra_guardrail_events=[input_check, quota_event]),
            }
            _persist_turn(conversation_id, current_user["_id"], original_question, response, blocked=True)
            return response

        classifier = IntentClassifier(api_key=API_KEY)
        result, timeout_event = _run_with_timeout(classifier.classify_intent, state.question, stage="intent_classification")
        if timeout_event:
            response = {
                ...
                "graph_response": _build_graph_response(state, extra_guardrail_events=[input_check, quota_event, timeout_event]),
            }
            _persist_turn(conversation_id, current_user["_id"], original_question, response, blocked=True)
            return response
        increment_usage(current_user["_id"], today, result.get("token_count", 0))
        model_events = result.get("guardrail_events", [])
        guardrail_events = [input_check, quota_event] + model_events
        ...
        if not intent_event["passed"]:
            ...
            response = {
                ...
                "answer": "I'm sorry, I couldn't determine your intent. This application is designed to answer questions related to warranty, user manuals, and inspection reports. Please ask a question related to one of these topics.",
        ...
        collection_name = result["intent"]
        if collection_name == "greetings":
            logger.info("Responded with greeting message")
            response = {
                "message": "Chat completed successfully",
                "answer": "Hello! How can I assist you today? Please ask a question related to warranty, user manuals, or inspection reports.",
            ...
            _persist_turn(conversation_id, current_user["_id"], original_question, response, blocked=False)
            return response
        collection_event = validate_collection_authorization(collection_name)
        guardrail_events = guardrail_events + [collection_event]
        if not collection_event["passed"]:
            logger.warning("Chat request blocked by collection authorization guardrail: %s", collection_event["reason"])
            response = {
                "message": "Request blocked by collection authorization",
                "answer": "I'm sorry, I couldn't route this question to a valid document collection.",
                "logs": [f"[guardrail:collection_authorization] BLOCKED - {collection_event['reason']}"],
                "graph_response": _build_graph_response(state, extra_guardrail_events=guardrail_events),
            }
            _persist_turn(conversation_id, current_user["_id"], original_question, response, blocked=True)
            return response

        cache_match, question_embedding = find_cache_match(current_user["_id"], collection_name, state.question, embedding_model)
        cache_event = {
        ...
        if cache_match:
            ...
            _persist_turn(
                ...
                collection_name=collection_name, cache_similarity=cache_match["similarity"],
                cache_source_message_id=cache_match["message_id"],
            )
            return response

        state.collection_name = collection_name
        compile, timeout_event = _run_with_timeout(compiled_graph.invoke, state, stage="answer_generation")
        if timeout_event:
            response = {
                "message": "Request timed out",
                "answer": "I'm sorry, this request took too long to process. Please try again.",
        ...
        logger.info("Chat completed successfully for collection '%s'", state.collection_name)
        ...
        _persist_turn(
            ...
            collection_name=collection_name if not blocked else None,
...
@router.delete("/clear/{collection_name}")
def clear_mongo_collection(collection_name: str, current_user: dict = Depends(_require_ragchatbot_access)):
    try:
        ...
        logger.exception("Error while clearing collection '%s': %s", collection_name, e)
        raise HTTPException(status_code=500, detail=str(e))


@router.delete("/delete/{collection_name}")
def delete_mongo_collection(collection_name: str, current_user: dict = Depends(_require_ragchatbot_access)):
    try:
        ...
        logger.exception("Error while deleting collection '%s': %s", collection_name, e)
        raise HTTPException(status_code=500, detail=str(e))
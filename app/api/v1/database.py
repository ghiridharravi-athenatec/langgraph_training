...
from app.core import config, db_connections, guardrail_config
...
def _get_owned_connection(connection_id: str, user_id: str) -> dict:
...
def chat_with_database(
    ...
    connection = _get_owned_connection(connection_id, current_user["_id"])
    ...
    result = run_db_agent(question, details, model=payload.model, history=history)
